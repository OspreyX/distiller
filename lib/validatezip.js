...
var validate = function(zf, callback) {
    checkFiles(zf.names, function(err, extensions, projectionFile) {
        ...
        return callback(null, extensions, projectionFile);
    });
};
...
var checkFiles = function(files, callback) {
    ...
    var projectionFile;
    if (files.length === 0) {
        return callback(invalid('Zipfile is empty'));
    }
    ...
    valid = filteredFiles.every(function(currentFile) {
        ...
        if (currentFile.indexOf('/') === -1) {
            ...
            filename = currentFile.slice((currentFile.indexOf('/') + 1), currentFile.indexOf('.'));
        }
    ...
    if (valid) {
        ...
        if (!hasRequiredFileTypes(extensions)) {
            return callback(invalid('Missing required file type(s)'));
        } else {
            return callback(null, extensions, projectionFile);
        }
    }
